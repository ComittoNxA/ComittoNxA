  ...
  if (BufSize>AllocSize)
  {
...
#else
      LOGE("Maximum allowed array size (%u) is exceeded",MaxSize);
#endif
    }
    ...
    if (Secure)
    {
      ...
      if (NewBuffer==NULL)
        ...
        ErrHandler.MemoryError();
#else
        LOGE("Array::Array - malloc error.(%u)", (unsigned int)(NewSize*sizeof(T)));
#endif
      if (Buffer!=NULL)
      {
    ...
    else
    {
      ...
      if (NewBuffer==NULL)
        ...
        ErrHandler.MemoryError();
#else
        LOGE("Array::Array - realloc error.(%u)", (unsigned int)(NewSize*sizeof(T)));
#endif
    }
    Buffer=NewBuffer;
    AllocSize=NewSize;
